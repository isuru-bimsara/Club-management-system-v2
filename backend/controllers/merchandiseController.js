...
//   // NEW: validation for sized products
...
//     finalSizes = parsedSizes.map((s) => ({ ...s, soldQuantity: 0 }));
...
//     // Keep sold quantities if same size already existed
...
//         throw new Error(`Quantity for size "${s.size}" cannot be lower than already sold (${prevSold})`);
//       }
...
//     const key = `sizes.${idx}.soldQuantity`;
//     const qtyKey = `sizes.${idx}.quantity`;

//     // atomic check+increment for size + global soldQuantity
//     updated = await Merchandise.findOneAndUpdate(
//       {
//         _id: merchandiseId,
//         $expr: {
//           $gte: [{ $subtract: [`$${qtyKey}`, { $ifNull: [`$${key}`, 0] }] }, qty],
//         },
//       },
//       { $inc: { [key]: qty, soldQuantity: qty } },
//       { new: true }
//     );
...
//     quantity: qty,
//     amount,
//     size: selectedSize, // empty string for non-sized products
//     receiptImage: `${uploadsBase}/receipts/${req.file.filename}`,
//     pickupVenue: merch.venue || merch.event?.venue || "",
//   });
...
//   if (previousStatus === "pending" && status === "rejected") {
//     if (order.merchandise.hasSizes && order.size) {
